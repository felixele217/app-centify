...
beforeEach(function () {
    ...
    $this->agent = Agent::factory()
        ...
        ->create([
            'email' => PipedriveHelper::demoSetByEmail($this->deals[0], env('PIPEDRIVE_DEMO_SET_BY')), // deals[0] is a demo scheduled deal
        ]);

});

it('auto-accepts deals if the flag is set in the organization', function () {
    $this->admin->organization->update(['auto_accept_demo_scheduled' => true]);

    (new PipedriveIntegrationService($this->admin->organization))->syncAgentDeals();

    expect($this->agent->deals->first()->sdr->pivot->accepted_at)->not()->toBeNull();
});

it('does not auto-accept deals if the flag is not set in the organization', function () {
    $this->admin->organization->update(['auto_accept_demo_scheduled' => false]);

    (new PipedriveIntegrationService($this->admin->organization))->syncAgentDeals();

    expect($this->agent->deals->first()->sdr->pivot->accepted_at)->toBeNull();
});
